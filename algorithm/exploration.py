        ...
        while self.robot.robotMode != "done":
            ...
            if self.timeLimit - explorationTime <= 20 or self.exploredArea == 300:
                if self.robot.isInStartZone():
                    ...
                    startnode = (self.robot.robotCenterH, self.robot.robotCenterW, int(
                        self.robot.robotHead))
                    endnode = (1, 1, 0)
                    (instructions, endOrientation) = dijkstra(
                        self.arena.get_2d_arr(), startnode, endnode, endOrientationImportant=False)
                    # give instruction
                    self.tcp_conn.send_command(instructions)
                    # update robot states (position and orientation)
                    (self.robot.robotCenterH, self.robot.robotCenterW,
                     self.robot.robotHead) = endnode
                    self.robot.robotMode = "done"
                    break
            else:
                if self.reachGoal:
            ...
            if 0 <= sensorIndex <= 4:
                ...
                for i in range(value):
                    ...
                    if (realTimeMap.get(h+offsets[i][0], w+offsets[i][1]) != CellType.OBSTACLE):
                        realTimeMap.set(
                            h+offsets[i][0], w+offsets[i][1], CellType.EMPTY)
                if value < self.MAX_SHORT_SENSOR:
                    if (19 >= h+offsets[value][0] >= 0 and 14 >= w+offsets[value][1] >= 0):
                        realTimeMap.set(
                            h+offsets[value][0], w+offsets[value][1], CellType.OBSTACLE)
            elif sensorIndex == 5:
    ...
    def reExplore(self):
        ...
        for cell in boundaryCells:
            euclideanDist = euclidean(
                [robot.robotCenterH, robot.robotCenterW], cell)
        ...
        offsets = [[-2, 1], [-2, 0], [-2, -1], [-1, -2], [0, -2],
                   [1, -2], [2, -1], [2, 0], [2, 1], [1, 2], [0, 2], [-1, 2]]
        potentialPos = []
        for offset in offsets:
            if self.allEmpty(targetCell[0]+offset[0], targetCell[1]+offset[1]):
                potentialPos.append(
                    [targetCell[0]+offset[0], targetCell[1]+offset[1]])
        # calculate Euclidean distance for each
        posDistance = []
        ...
        # use djikstra
        # change to int(robothead) because somehow the robotHead is a float
        startnode = (robot.robotCenterH, robot.robotCenterW,
                     int(robot.robotHead))
        (instr, endNode) = dijkstra(self.arena.get_2d_arr(),
                                    startnode, cellToMove, endOrientationImportant=True)
